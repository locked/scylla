...
future<redis_message> hgetall::execute(service::storage_proxy& proxy, redis::redis_options& options, service_permit permit) {
    return redis::read_list_from_hash(proxy, options, _key, permit).then([] (auto result) {
        if (result->has_result()) {
            return redis_message::make_list_result(std::move(result->results()));
        }
        // return nil string if key does not exist
        return redis_message::nil();
    });
...
future<redis_message> hget::execute(service::storage_proxy& proxy, redis::redis_options& options, service_permit permit) {
    return redis::read_strings_from_hash(proxy, options, _key, _field, permit).then([this] (auto result) {
        if (result->has_result()) {
            return redis_message::make_strings_result(std::move(result->results().at(1)));
        }
        // return nil string if key does not exist
        return redis_message::nil();
    });
...
shared_ptr<abstract_command> hdel::prepare(service::storage_proxy& proxy, request&& req) {
    ...
    return seastar::make_shared<hdel> (std::move(req._command), std::move(req._args[0]), std::move(std::vector<bytes>(req._args.begin() + 1, req._args.end())));
}
